#!/usr/bin/env python3
"""
MMI — Fase 2 · Reindexado del lote 1 con el chunker refinado.

Borra cada documento (Postgres + Qdrant) y lo vuelve a ingerir con el chunker
de la Fase 2 (secciones, cortes semánticos, guardas refinadas).
"""
import os
import sys

sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "pipeline"))
from ingest import reingest
...
def main():
    ...
    for fname, tipo, version, dominio in ARCHIVOS:
        ...
        if not os.path.exists(path):
            ...
            continue
        try:
            res = reingest(path, TENANT, tipo, version, dominio)
        except Exception as e:
            res = {"archivo": fname, "estado": "error", "detalle": str(e)[:200]}
        resultados.append(res)
        print(res)

    print("\n=== RESUMEN REINDEXADO LOTE 1 ===")
