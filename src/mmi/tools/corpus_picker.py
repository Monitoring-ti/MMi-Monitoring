...
from mmi.ingest.file_types import FILE_TYPES
...
def _render_file_row(it: CorpusItem) -> str:
    ...
    return f"""<div class="file-row" data-ext="{it.extension}" data-source="{it.source}" data-proc="{proc}"
     data-folder="{_esc('/'.join(it.relative_path.replace(chr(92), '/').split('/')[:-1]))}">
...
def _render_folder_node(node: FolderNode, depth: int = 0, open_depth: int = 2) -> str:
  ...
  <summary>
    ...
    <span class="folder-count">{count} archivo{"s" if count != 1 else ""}</span>
  </summary>
...
<style>
  ...
  .tree-header {{ display: grid; grid-template-columns: 28px 72px 1.2fr 2fr 56px 72px 1fr;
  ...
  .file-row {{
    display: grid; grid-template-columns: 28px 72px 1.2fr 2fr 56px 72px 1fr;
...
<body>
  ...
  <p class="meta">Local: {local_n} · SharePoint (catálogo): {sp_n} · Procesables: {proc_n}.
     Los de SharePoint sin ruta local no se pueden embeber hasta sync/descarga.</p>
  <div class="toolbar">
    <input id="q" type="search" placeholder="Filtrar por nombre o ruta…" style="min-width:260px"/>
    <select id="ext">
    ...
    <button type="button" class="primary" id="save">Guardar selección</button>
  </div>
  <div class="tree-header">
    <span></span><span>Fuente</span><span>Nombre</span><span>Ruta</span><span>Ext</span><span>Tamaño</span><span>Notas</span>
  </div>
  <div class="tree" id="tree">
    {tree_html}
  </div>
  <div id="status">Marca archivos y pulsa <b>Guardar selección</b>. Se escribe <code>out/process-manifest.json</code> (o se descarga si el servidor no responde).</div>
<script>
...
function applyFilter() {{
  ...
  const e = ext.value;
  const s = src.value;
  const hasFilter = Boolean(qq || e || s);
  ...
  document.querySelectorAll('.file-row').forEach(row => {{
    ...
    row.classList.toggle('hidden', !(okQ && okE && okS));
  }});
...
function selectedPayload() {{
  return [...document.querySelectorAll('input[name=sel]:checked')].map(cb => ({{
    id: cb.value,
...
function downloadFallback(selected) {{
  const payload = {{
    created_at: new Date().toISOString(),
    count: selected.length,
    local_ready: selected.filter(f => f.absolute_path).length,
    online_only: selected.filter(f => !f.absolute_path).length,
    files: selected,
  }};
  ...
  a.click();
  status.className = 'ok';
  status.textContent = 'Descargado process-manifest.json (mueve el archivo a out/).\\n' +
...
document.getElementById('save').onclick = async () => {{
  ...
  try {{
    ...
    status.textContent = `Guardado: ${{data.path}}\\n` +
      `${{data.count}} archivos (${{data.local_ready}} con ruta local, ` +
      `${{data.online_only}} solo SharePoint).`;
  }} catch (err) {{
    downloadFallback(selected);
  }}
}};
</script>
</body>
...
def write_manifest(selected: list[dict], out_path: Path, inventory: list[CorpusItem]) -> dict:
        ...
        files.append(
            {
                ...
                "ready": bool(abs_path),
                "suggested_tipo": _suggest_tipo(row.get("name", ""), row.get("extension", "")),
            }
        )
    payload = {
    ...
    return {"path": str(out_path), **{k: payload[k] for k in ("count", "local_ready", "online_only")}}
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                result = write_manifest(data.get("selected") or [], manifest_path, inventory)
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"Inventario: {len(inventory)} items")
    for it in inventory:
        mark = "+" if it.processable else "-"
        print(f"  {mark} [{it.source}] {it.relative_path}")
    ...
    if args.serve:
        handler = make_handler(inventory, args.manifest.resolve())
